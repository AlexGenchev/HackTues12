...
def fill_vik_form(voice_text):
    """
    Превръща речта в официален JSON и попълва формата.
    """
    # --- СТЪПКА 1: AI ОБРАБОТКА ---
    print("AI анализира сигнала...")
    prompt = f"""
    Ти си администратор във ВиК. Превърни разговорния текст в официален сигнал.
    Извлечи данни в JSON формат:
    {{
        "name": "име на подателя",
        "village": "точно име на селото с 'с.' отпред, напр. 'с. Владо Тричков'",
        "address": "точен адрес",
        "issue_type": "Нарушено водоснабдяване" или "Друг проблем",
        "official_description": "Професионално написан текст на сигнала на български език"
    }}
    Текст: "{voice_text}"
    """
    ...
    # --- СТЪПКА 2: PLAYWRIGHT ---
    # Търсим общината в речника
    village_key = ai_data['village'].lower().replace("с. ", "").strip()
    location_info = LOCATIONS.get(village_key, {"municipality": "Своге"})
    
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False) # Виждаме го за демото
        page = browser.new_page()
        
        try:
            print(f"🚀 Попълване на сигнал за {ai_data['village']}...")
            page.goto("https://viksofbg.com/signal/")
            
            # Име, Мейл, Телефон
            page.get_by_placeholder("Име").fill(ai_data['name'])
            page.get_by_placeholder("Имейл").fill("bojidarmarkov3.0@gmail.com")
            page.get_by_placeholder("Телефон").fill("0882482180")
            
            # Избор на Район
            page.get_by_label("Район").select_option(label=location_info['municipality'])
            time.sleep(1.5) 
            
            # Избор на Населено място
            page.get_by_label("Населено място").select_option(label=ai_data['village'])
            
            # Адрес
            page.get_by_placeholder("Точен адрес").fill(ai_data['address'])
            
            # ОФИЦИАЛНО ОПИСАНИЕ (от AI)
            print(f"📝 Описание: {ai_data['official_description']}")
            page.get_by_placeholder("Описание").fill(ai_data['official_description'])
            
            # Тип проблем
           # 5. ТИП ПРОБЛЕМ - Винаги избираме "Друг проблем"
            print("Избирам опция: Друг проблем")
            page.get_by_label("Друг проблем").check()
            page.get_by_placeholder("Описание").fill(ai_data['official_description'])
